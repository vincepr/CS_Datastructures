...
namespace src.datastructures.Queue
{
    ...
    public sealed class QueueUsingLinkedList<T> : IEnumerable<T> where T : IComparable<T>
    {
        ...
        public T? PeekFirst() => list.First is null ? default : list.First.Value;    // the default(T) should never get reached

        public T? PeekLast() => list.Last is null ? default : list.Last.Value;
        ...
        public T? Remove()
        {
            if (IsEmpty()) throw new InvalidOperationException("Cant Remove from empty Queue");
